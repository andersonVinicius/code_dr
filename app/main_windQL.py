...
for s in range(len(read[100:101])):
    np.random.seed(read[s])
    print('Seed current:',s)
    uavVel = 20 # m/s
    list_grid = []
    count = 0
    ...
    #event wind==========================================================
    BlockInterference = np.round(N_blocks*0.9).astype(int) # 10%
    ...
    def calNeighborhood(id,x,y,z,z_point):
        ...
        return actions
    r = [-1,0,1] # yes wind - no_wind - no_windBest
    #create states
    for z in range(zdim):
        for y in range(ydim):
            for x in range(xdim):
                    list_grid.append(Grid_space(count, x, x+1, y, y+1, z, z+1,
                                                calNeighborhood(count,xdim,ydim,zdim,z),r[1]))
                    count = count+1
    ...
    for i in blockWithoutWind:
        if list_grid[init_space].id != list_grid[i].id:
            p1=np.array([list_grid[init_space].x_init, list_grid[init_space].y_init, list_grid[init_space].z_init])
...
# np.savetxt("/home/anderson/Dropbox/posGraduacao/doutorado/qualis_paper/dataS/seedDisponibles.csv", seedDisponible, delimiter=";")

# print(read.astype(int))
# print('quant elements:',len(read))
#====================================================================================================
